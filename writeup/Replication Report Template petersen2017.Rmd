...
First, I anticipate to calculate between-group error in recalls for the treatment condition and control condition. I will also calculate the linear regression model for the within-group errors as part of my exploratory analysis. 
...
``` {r}
knitr::include_graphics("originalgraph.png")
#This original graph doesn't show the effect of control versus experimental condition. I will attempt to create a graph from 
```
```{r echo=TRUE}
betweengraph<-ggplot(dataforanalysis, aes(x=Condition, y=betweenerrorcount, fill=Condition)) +
...
summary(Modelwithin)
#Graphing it
Withingraph<-ggplot(dataforanalysis, aes(x=Condition, y=withinerrorcount, fill=Condition)) +
geom_bar(position="dodge",stat="summary", fun.y= "mean") + 
labs(title = "Within-group error or confusion of faces within the same race", y = "Within error count", x= "Conditions")+
  scale_x_discrete(labels=c("Control", "Experimental"))+
scale_fill_brewer(palette="Set1",labels= c("Control", "Experimental"))+
  guides(fill=F) + #Hide legends
  theme_bw() +
